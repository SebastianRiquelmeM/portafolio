	...
	return (
		<motion.button
			...
			className={`relative inline-flex h-10 w-20 items-center justify-center rounded-full ${
				darkSide ? 'bg-indigo-600' : 'bg-gray-200'
		...
		>
			...
			<motion.span
				className={`absolute left-1 flex h-8 w-8 items-center justify-center rounded-full ${
					darkSide ? 'bg-white translate-x-10' : 'bg-white'
				} shadow transition-transform duration-300`}
				animate={{ x: darkSide ? 36 : 0 }}
			...
			<span className={`absolute right-2 transition-opacity duration-300 ${darkSide ? 'opacity-0' : 'opacity-100'}`}>
				<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
				</svg>
			</span>
			
			<span className={`absolute left-2 transition-opacity duration-300 ${darkSide ? 'opacity-100' : 'opacity-0'}`}>
